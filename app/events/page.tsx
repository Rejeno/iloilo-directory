...
export const EventCard = ({ title, date, description, author, imageUrl, avatarUrl, status }: EventCardProps) => {
    return (
        <div className="bg-[#FCFCF5] rounded-[12px_24px_24px_24px] p-2 shadow-[0px_4px_10px_rgba(0,0,0,0.1)] flex flex-col h-full transition-transform hover:-translate-y-1 cursor-pointer">
            {/* Image */}
            ...
            {/* Details */}
            <div className="mt-3 px-2 pb-1 flex flex-col flex-grow">
                <div className="flex justify-between items-center mb-1.5">
                    <div className="flex items-center gap-1.5 text-[#4D4D4D] text-xs font-wix">
                        <Calendar size={13} className="text-[#343434]" />
                        <span>{date}</span>
                    </div>
                    <span className="bg-[#113FC8] text-[#FAFAEF] px-3 py-0.5 rounded-md text-[10px] font-bold font-sora">
                        {status}
                    </span>
                </div>

                <h3 className="font-wix text-lg font-medium text-[#343434] truncate mb-1">
                    {title}
                </h3>

                <p className="text-[#4D4D4D] text-xs font-wix leading-[18px] mb-3 flex-grow line-clamp-3">
                    {description}
                </p>

                <div className="flex justify-between items-center mt-auto pt-2 border-t border-[#DCDCD6]">
                    <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[#4D4D4D] text-[10px] font-wix max-w-[100px] truncate">
                            {author}
                        </span>
                    </div>

                    <button className="flex items-center gap-1.5 border border-[#113FC8] text-[#113FC8] px-3 py-1 rounded-xl text-xs font-bold font-sora hover:bg-[#113FC8] hover:text-white transition-colors">
                        View Details
                        <ArrowRight size={11} />
                    </button>
                </div>
...
export default function EventsPage() {
    ...
    return (
        <main className="bg-[#FBFBF5] min-h-screen pb-20">
            {/* Hero Section — matched to Classifieds h-[450px] + text-5xl/6xl */}
            <section className="relative w-full h-[450px] flex flex-col items-center justify-center text-[#FCFCF5] overflow-hidden">
                <div
                    className="absolute inset-0 bg-cover bg-center"
                    style={{
                        backgroundImage: `linear-gradient(0deg, rgba(20, 17, 17, 0.52), rgba(20, 17, 17, 0.52)), url('/hero-bg.jpg')`,
                        zIndex: 0
                    }}
                />
            ...
            {/* Main Content — matched to Classifieds max-w-[1280px] + px-8/12 + mt-16 */}
            <section className="max-w-[1280px] mx-auto px-8 md:px-12 mt-16 pb-24">
                {/* Results Info & Sort — matched to Classifieds layout */}
                <div className="flex flex-col md:flex-row justify-between items-end mb-8 gap-6">
                    ...
                    <div className="flex items-center gap-4">
                        {/* Sort dropdown — matched to Classifieds w-[190px] h-[44px] */}
                        ...
                        {/* View Toggles — matched to Classifieds w-10 h-10 */}
                ...
                {/* Events Grid — matched to Classifieds grid-cols-1/2/3/4 gap-6 */}
                ...
                {/* Pagination — matched to Classifieds Pagination component */}
